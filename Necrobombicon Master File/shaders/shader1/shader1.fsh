...
uniform vec2 pixel; // 2.0,2.0
uniform float Freq; // = 0.115;
uniform vec2 resolution;// render target w/h
...
vec4 spline(float x, vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec4 c5, vec4 c6, vec4 c7, vec4 c8, vec4 c9)
{
  ...
  w1 = 0.0;  w2 = 0.0;  w3 = 0.0;  w4 = 0.0;  w5 = 0.0;  w6 = 0.0;  w7 = 0.0;  w8 = 0.0;  w9 = 0.0;
  float tmp = x * 8.0;
  if (tmp<=1.0) { w1 = 1.0 - tmp; w2 = tmp; }
  else if (tmp<=2.0) { tmp = tmp - 1.0; w2 = 1.0-tmp; w3 = tmp; }
  else if (tmp<=3.0) { tmp = tmp - 2.0; w3 = 1.0-tmp; w4 = tmp; }
  else if (tmp<=4.0) { tmp = tmp - 3.0; w4 = 1.0-tmp; w5 = tmp; }
  else if (tmp<=5.0) { tmp = tmp - 4.0; w5 = 1.0-tmp; w6 = tmp; }
  else if (tmp<=6.0) { tmp = tmp - 5.0; w6 = 1.0-tmp; w7 = tmp; }
  else if (tmp<=7.0) { tmp = tmp - 6.0; w7 = 1.0-tmp; w8 = tmp; }
  else {
...
vec3 NOISE2D(vec2 p) { return texture2D(NoiseTex,p).xyz; }
...
void main()
{
  ...
  {
    vec2 Delta = vec2(pixel.x/resolution.x,pixel.y/resolution.y);
    vec2 ox = vec2(Delta.x,0.0);
    vec2 oy = vec2(0.0,Delta.y);
    vec2 PP = uv - oy;
